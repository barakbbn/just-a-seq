  ...
  readonly run= () => describe('SeqBase - Deferred Execution', () => {
      ...
      describe('split()', () => {
        ...
        describe('by condition', () => {
          it.skip('TODO: should return two sequences, first one with items while the condition met and the second with the rest', () => {
      ...
      describe('tap()', () => {
        it('should call callback for each item in source sequence with correct index', () => {
          const input = array.zeroToNine;
          const expected = input.map((x, i) => ({
            item: x,
            index: i
          }));

          const actual: { item: number; index: number; }[] = [];

          asSeq(input).tap((item, index) => actual.push({item, index})).consume();
