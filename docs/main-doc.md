...
<details>
  ...
  <h3><code>filter(condition: Condition&lt;T&gt;): Seq&lt;T&gt;</code></h3>
  <dl>
    <dt>- condition</dt>
    <dd>Function to test each item and returns a truthy value if should keep the item or or falsy value if to remove it</dd>
  </dl>
  <h3><code>  filter&lt;S extends T&gt;(typeGuard: (item: T, index: number) => item is S, thisArg?: any): Seq&lt;S&gt;</code></h3>
  Filter and coerce to Typescript type using type-guard function
    <dl>
    <dt>- typeGuard</dt>
    <dd>Typescript type-guard function to check the item is of desired type and cast the returned sequence to that type</dd>
  </dl>
  <details>
    <summary>Example</summary>

```typescript
import {asSeq, Seq} from 'just-a-seq';
...
interface RangeValidator extends Validator<number> {
  readonly type: 'Range';
  range: { from: number; to: number; };
}

interface ValidatorContainer<T> extends Validator<T> {
  readonly type: 'Container';
  validators: Validator<T>[];
}

const validators: Validator[] = loadValidators();

const ranges = asSeq(validators)
  .filter((validator): validator is RangeValidator => validator.type === 'Range')
  .map(validator => validator.range);
```
  </details>
  <hr>
</details>
...
<!-- repeat() -->
<details>
  <summary><samp><b>repeat()</b></samp> - Multiply the sequence</summary>
...
<!-- reverse() -->
<details>
  <summary><samp><b>reverse()</b></samp> - <small><i>(Like Array.reverse)</i></small> Reverse the order of the sequence</summary>
